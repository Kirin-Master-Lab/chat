...
/**
 * 按 session_id + assistant_type 持久化聊天记忆。
 */
@Component
public class CustomChatMemoryStore implements ChatMemoryStore {

    static final String MEMORY_ID_SEPARATOR = "::";
    static final String ASSISTANT_TYPE_DEFAULT = "default";
    ...
    public List<ChatMessage> getMessages(Object memoryId) {
        ...
        ChatMemoryEntity entity = chatMemoryMapper.selectOne(buildMemoryQuery(memoryContext));
        if (entity == null) {
            return new ArrayList<>();
        }
    ...
    public void updateMessages(Object memoryId, List<ChatMessage> messages) {
        ...
        ChatMemoryEntity entity = chatMemoryMapper.selectOne(buildMemoryQuery(memoryContext));
        if (entity == null) {
            ChatSessionEntity session = chatSessionMapper.selectById(memoryContext.sessionId());
            if (session == null) {
                return;
            }
            ...
            entity.setSessionId(memoryContext.sessionId());
            entity.setAssistantType(memoryContext.assistantType());
    ...
    public void deleteMessages(Object memoryId) {
        ...
        chatMemoryMapper.delete(buildMemoryQuery(memoryContext));
    }

    private LambdaQueryWrapper<ChatMemoryEntity> buildMemoryQuery(MemoryContext memoryContext) {
        return new LambdaQueryWrapper<ChatMemoryEntity>()
                .eq(ChatMemoryEntity::getSessionId, memoryContext.sessionId())
                .eq(ChatMemoryEntity::getAssistantType, memoryContext.assistantType());
    }
    ...
    private MemoryContext parseMemoryContext(Object memoryId) {
        ...
        Long sessionId;
        try {
            sessionId = Long.valueOf(parts[0].trim());
        ...
        return new MemoryContext(sessionId, assistantType);
    }

    record MemoryContext(Long sessionId, String assistantType) {
    }
}
